<template>
  <div class="user-care-wrapper" ref="usercareelem">
    <nav-bar>
      <i class="iconfont iconfanhui" slot="left" @click="backHistory"></i>
      <span slot="center" v-show="userName">{{ userName }}的关注</span>
    </nav-bar>

    <user-lists-show :user-info-lists="userCareLists" @click.native="onClickCareList"></user-lists-show>
    <div class="clear-fix">
      <span
        ...
        >更多</span
      >
      <span class="more-care-btn right"   style="color: #666; font-size: 0.6em; margin-right: 0.4em" v-else>没有更多了</span>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    getUserCareLists() {
       ...
       return getUserCareById(this.userId, this.curCareCount).then((lists) => {
          //  console.log("获取到用户", this.userName, "的关注", lists);
          if (lists.length === 0) {
            this.hasMore = false;
            return;
          }
